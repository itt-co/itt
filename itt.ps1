    ...
    Invoke-ScriptBlock -ArgumentList $Debug -ScriptBlock {
        ...
        function Get-UsersCount {
            if (-not (Test-Path $itt.registryPath)) {
                New-Item -Path $itt.registryPath -PropertyType DWORD -Force | Out-Null
            }
            $currentValue = Get-ItemPropertyValue -Path $itt.registryPath -Name "Runs" -ErrorAction SilentlyContinue
