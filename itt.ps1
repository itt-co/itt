...
function Invoke-Install {
   ...
   if($result -eq "no") {
    ...
    Invoke-ScriptBlock -ArgumentList $selectedApps $total -debug $debug -ScriptBlock {

        param($selectedApps ,$total ,$debug)
        ...
        $selectedApps | ForEach-Object {
            ...
            if ($_.Winget -ne "none" -or $_.Choco -ne "none")
            {
                ...
                Install-App -appName $_.Name -appWinget $_.Winget -appChoco $_.Choco -TotalInstalled $total.Count -InstalledList $selectedApps.name
